...
vec3 skyColor(float h) {
    ...
    vec3 c2 = vec3(0.20, 0.50, 0.85);
    vec3 c3 = vec3(0.60, 0.82, 0.97);
    vec3 c4 = vec3(0.95, 0.97, 1.00);
...
float getHeight(vec2 c, vec2 z, vec2 p1) {
    ...
    vec2 tc  = vec2((p1.x - 1.0 - 1.7320508*p1.y)*0.5,
                    (p1.y + 1.7320508*(p1.x+1.0))*0.5);
    for (int i = 0; i < MAX_ITER; i++) {
        vec2 oldZ = z;
        vec2 uvNorm = fract(z / 20.0 + 0.5);
