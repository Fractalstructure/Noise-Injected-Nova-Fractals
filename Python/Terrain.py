...
float hash21(vec2 p) {
    vec2 q = fract(p * vec2(12.9898, 78.233));
    q += dot(q, q + 34.45);
...
void main() {
    ...
    vec2 z = (fragCoord / iResolution.xy - 0.5) * 8.0;
    float total = 0.0;
    float amp   = 1.0;
    float freq  = 1.0;
    for (int k = 0; k < 6; k++) {
        total += (valueNoise(z * freq) * 2.0 - 1.0) * amp;
        freq  *= 2.5;
        amp   *= 0.5;
    }
    float n = (total + 1.0) * 0.3;
...
vec3 iceColor(float h) {
    ...
    vec3 c2 = vec3(0.15, 0.45, 0.65);
    vec3 c3 = vec3(0.70, 0.85, 0.95);
    vec3 c4 = vec3(0.98, 1.00, 1.00);
...
void main() {
    ...
    vec3 col    = iceColor(h) * (1.0 + 0.01*h);
